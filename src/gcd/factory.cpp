...
gcode_interpreter_objects gcode_interpreter_objects_factory(const configuration::global &cfg, const machine_driver_selection driver_select_) {
    gcode_interpreter_objects gio_ret;
//std::shared_ptr<hardware::motor_layout> motor_layout;

    if (driver_select_ == RASPBERRY_PI) {
        try {
            gio_ret.hardware_driver = std::make_shared< hardware::driver::raspberry_pi_3>(cfg);
        } catch ( ... ) {
            std::cout << "falling back to emulation of hardware motors..." << std::endl;
            gio_ret.hardware_driver = std::make_shared< hardware::driver::inmem>();
        }
    } else {
        gio_ret.hardware_driver = std::make_shared< hardware::driver::inmem>();
    }
    
    gio_ret.motor_layout = hardware::motor_layout::get_instance(cfg);
    movement::steps_generator steps_generator_drv(gio_ret.motor_layout);
    
    
    gio_ret.stepping = std::make_shared<hardware::stepping_simple_timer>(cfg, gio_ret.hardware_driver);



	movement::variable_speed variable_speed_driver( gio_ret.motor_layout, cfg, cfg.tick_duration() );
