...
public:
    const std::vector<executor_command_t> get_motion_plan() const
    {
        ...
        for (unsigned int i = 1; i < motion_fragments_vector.size()-1; i++) {
            ...
            if (coeffA > 0.5) prev_speed_dst = _cfg->layout.max_no_accel_velocity_mm_s;
            else prev_speed_dst = (coeffA*_cfg->layout.max_no_accel_velocity_mm_s + (0.5 - coeffA)*prev_speed_dst)/0.5;
...
int main(int argc, char** argv)
{
    ...
    //executor.execute(generate_sin_wave_for_test());
    {
        ...
        mp.gotoxy(distance_t{5.0, 0.0, 0.0, 0.0}, 20.0)
            ...
            .gotoxy(distance_t{5.0, -5, 0.0, 0.0}, 20.0)
            .gotoxy(distance_t{0.0, -5.0, 0.0, 0.0}, 20.0)
