...
void variable_speed::set_motor_layout(std::shared_ptr<hardware::motor_layout> ml)
{
    _motor_layout_ptr = ml;
    _motor_layout = _motor_layout_ptr.get();
}

variable_speed::constant_speed(std::shared_ptr<hardware::motor_layout> ml)
{
    set_motor_layout(ml);
}
/**
     ...
     */
std::vector<hardware::multistep_command> variable_speed::goto_xyz(const distance_t p0, const distance_t p1, double v0, double dv double dt)
{
