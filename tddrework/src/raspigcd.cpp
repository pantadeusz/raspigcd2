...
int main() {
    ...
    for (int i = 0; i < cfg.steppers[2].steps_per_mm*2; i++) {
        ...
        stp.cmnd.repeat = 1;
        for (int e = 0; e < RASPIGCD_HARDWARE_DOF; e++) stp.cmnd.b[e] = fwd_cmnd[e];
        commands.push_back(stp);
        stp.cmnd.repeat = 20;
        for (int e = 0; e < RASPIGCD_HARDWARE_DOF; e++) stp.cmnd.b[e] = zero_cmnd[e];
        commands.push_back(stp);
    }
    for (int i = 0; i < cfg.steppers[2].steps_per_mm*2; i++) {
        multistep_command stp;
        stp.cmnd.repeat = 1;
        for (int e = 0; e < RASPIGCD_HARDWARE_DOF; e++) stp.cmnd.b[e] = back_cmnd[e];
        commands.push_back(stp);
        stp.cmnd.repeat = 20;
        for (int e = 0; e < RASPIGCD_HARDWARE_DOF; e++) stp.cmnd.b[e] = zero_cmnd[e];
        commands.push_back(stp);
